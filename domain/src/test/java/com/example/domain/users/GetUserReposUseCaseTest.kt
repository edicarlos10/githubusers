...
class GetUserReposUseCaseTest : BaseTest() {
    @RelaxedMockK
    ...
    @Test
    fun `Should return user repo`() {

        every { repository.getUserRepos(any()) } returns
    ...
    @Test
    fun `Should return user repo error`() {
        every { repository.getUserRepos(any()) } returns Single.error(mockErrorResponse)
