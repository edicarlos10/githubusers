...
import com.example.network.remote.response.LicenseResponse
import com.example.network.remote.response.OwnerResponse
...
class UsersRemoteDataTest : BaseTest() {
    ...
    private fun responseUserDetail(): Response<UsersResponse> {
        return Response.success(
            UsersResponse(
                id = 1,
                type = "User",
    ...
    private fun responseUserRepos(): Response<List<UserReposResponse>> {
        return Response.success(
            arrayListOf(
                UserReposResponse(
    ...
    fun `Should get user detail from API`() {
        ...
        remoteData.getUserDetail("mojombo")
            .test()
            .assertValue(responseUserDetail().body()?.toUsers())
    }

    @Test
    fun `Should get user repos from API`() {

        every { client.getUserRepos(any()) } returns
                Single.just(responseUserRepos())

        remoteData.getUserRepos("mojombo")
            .test()
            .assertValue(responseUserRepos().body()?.map { it.toUsersRepos() })
    }
}